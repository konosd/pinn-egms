...
if __name__ == '__main__':
    ...
    model_files = ([f for f in os.listdir(os.path.join(results_directory)) if 'pt' in f])
    model_files.sort(key=lambda x: os.path.getmtime(os.path.join(results_directory,x)))


    trainer.model_u = torch.load(os.path.join(results_directory, model_files[-1]))
    print(f'Loded model {model_files[-1]}')
