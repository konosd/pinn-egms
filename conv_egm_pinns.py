...
#         y_d.reshape(-1,1)
    ...
    def  heterogeneous_pde_loss(self, x, u, d):
        ...
        dw = torch.autograd.grad(w, x, grad_outputs = torch.ones_like(u), retain_graph=True)[0]
        dv = torch.autograd.grad(v, x, grad_outputs = torch.ones_like(u), retain_graph=True)[0]
...
class ae(nn.Module):
    def __init__(self):
        ...
        self.deconv1 = nn.ConvTranspose2d(64, 32, kernel_size = 3)
        self.batchnorm4 = nn.BatchNorm2d(32)
        self.deconv2 = nn.ConvTranspose2d(32, 16, kernel_size = 3)
        self.batchnorm5 = nn.BatchNorm2d(16)
        self.deconv3 = nn.ConvTranspose2d(16, 3, kernel_size = 3)
    ...
    def forward(self, x):
        ...
        x = self.activation(self.batchnorm4(self.deconv1(x)))
        x = self.activation(self.batchnorm5(self.deconv2(x)))
        x = self.deconv3(x)
        return x
...
class egm_sampler():
    ...
    def __getitem__(self, idx):
        x = torch.as_tensor(self.inputs[idx], dtype = torch.float)
        y = torch.as_tensor(self.outputs[idx], dtype = torch.float)
...
class model_trainer_egm_homogeneous():
    def __init__(self, model_u, train_data, test_data, batch_size, geomtime, bc, loss, system):
        ...
        self.pinn_test = DataLoader(self.test_data, batch_size = batch_size, num_workers = 0)

        self.pinn_train = DataLoader(self.train_data, batch_size = batch_size, num_workers = 0) 

        self.d = self.train_data[0][1][3]
    ...
    def train(self, epochs, learning_rate, draw_figure = False, savedir = None, loss_weights = [0,0,0,0], verbose = True):
        ...
        for epoch in range(epochs):
            ...
            for i, batch in enumerate(self.pinn_train):
                ...
                dv_y, dv_x = torch.gradient(u_i_hat[:,2], spacing=0.3125, dim=(1,2))
                dv_yy = torch.gradient(dv_y, spacing = 0.3125, dim=2)[0]
                dv_xx = torch.gradient(dv_x, spacing = 0.3125, dim=1)[0]
                ddv = (dv_yy + dv_xx)

                phi_hat = torch.nn.functional.conv2d( ddv[:,np.newaxis], self.r_egm, padding = 'same')

                # Now for the boundary, check which are on the boundary:
                dvdx = torch.autograd.grad(u_i_hat[:,0], x_i, grad_outputs = torch.ones_like(u_i_hat[:,0]), retain_graph = True)[0]
                dwdx = torch.autograd.grad(u_i_hat[:,1], x_i, grad_outputs = torch.ones_like(u_i_hat[:,0]), retain_graph = True)[0]
                dudx = torch.autograd.grad(u_i_hat[:,2], x_i, grad_outputs = torch.ones_like(u_i_hat[:,0]), retain_graph = True)[0]

                bc_loss = torch.mean(abs(dvdx[:,0,:,0]+dvdx[:,0,:,-1]+dvdx[:,1,0,:]+dvdx[:,1,-1,:]+ dwdx[:,0,:,0]+dwdx[:,0,:,-1]+dwdx[:,1,0,:]+dwdx[:,1,-1,:]+ dudx[:,0,:,0]+dudx[:,0,:,-1]+dudx[:,1,0,:]+dudx[:,1,-1,:]))
                ...
                phi_loss = self.loss(phi_hat, u_i[:,4])
            ...
            if epoch%(epochs//100)==0:
                ...
                if draw_figure==True:
                    ...
                    im = ax.imshow(u_i[-1][4].detach().cpu().numpy())
                    ax.set_title('EGM ground truth')
                    plt.colorbar(im, ax = ax)
                    
                    ax =plt.subplot(2,4,2)
                    im = ax.imshow(u_i_hat[-1,2].detach().cpu().numpy())
                    ax.set_title('AP prediction')
                    plt.colorbar(im, ax = ax)
                    
                    ax =plt.subplot(2,4,6)
                    im = ax.imshow(u_i[-1][2].detach().cpu().numpy())
                    ax.set_title('AP ground truth')
                    plt.colorbar(im, ax = ax)
                    
                    ax =plt.subplot(2,4,3)
                    im = ax.imshow(u_i_hat[-1,0].detach().cpu().numpy())
                    ...
                    im = ax.imshow(u_i_hat[-1,1].detach().cpu().numpy())
                    ax.set_title('w prediction')
                    plt.colorbar(im, ax = ax)
                    
                    ax =plt.subplot(2,4,8)
                    im = ax.imshow(u_i[-1][1].detach().cpu().numpy())
